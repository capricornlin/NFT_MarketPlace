...
import { NFTCards, Loader, Banner, SearchBar } from '../components';
...
const MyNFTs = () => {
  ...
  const [nftCpoy, setNftCpoy] = useState([]);

  const [isLoading, setIsLoading] = useState(false);
  const [activeSelect, setActiveSelect] = useState('Recently Added');

  //NOTE:dependency要用currentAccount
  useEffect(() => {
    // > return a Promise
  ...
  return (
    <div className="w-full flex justify-start items-center flex-col min-h-screen">
      ...
      {!isLoading && !nfts.length && !nftCpoy.length ? (
        <div className="flexCenter sm:p-4 p-16">
          <h1 className='className="font-poppins dark:text-white text-nft-black-1 font-extrabold text-2xl "'>
            No NFTs Owned
          </h1>
        </div>
      ) : (
        <div className="sm:p-4 p-12 w-full minmd:w-4/5 flexCenter flex-col">
          ...
          <div className="mt-3 w-full flex flex-wrap">
            {nfts.map((nft) => (
              <NFTCards key={nft.tokenId} nft={nft} onProfilePage />
            ))}
          </div>
        </div>
      )}
